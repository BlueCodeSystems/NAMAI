package org.smartregister.anc.library.reporting.dropuout.intent;
...
import org.smartregister.anc.library.reporting.dropuout.domain.Cohort;
import org.smartregister.anc.library.reporting.dropuout.domain.CohortIndicator;
import org.smartregister.anc.library.reporting.dropuout.domain.CohortPatient;
import org.smartregister.anc.library.reporting.dropuout.domain.Cumulative;
import org.smartregister.anc.library.reporting.dropuout.domain.CumulativeIndicator;
import org.smartregister.anc.library.reporting.dropuout.domain.CumulativePatient;
import org.smartregister.anc.library.reporting.dropuout.receiver.CoverageDropoutBroadcastReceiver;
import org.smartregister.anc.library.reporting.dropuout.repository.CohortIndicatorRepository;
import org.smartregister.anc.library.reporting.dropuout.repository.CohortPatientRepository;
import org.smartregister.anc.library.reporting.dropuout.repository.CohortRepository;
import org.smartregister.anc.library.reporting.dropuout.repository.CumulativeIndicatorRepository;
import org.smartregister.anc.library.reporting.dropuout.repository.CumulativePatientRepository;
import org.smartregister.anc.library.reporting.dropuout.repository.CumulativeRepository;
import org.smartregister.anc.library.util.AppConstants;
...
public class CoverageDropoutIntentService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        Timber.i("Started Coverage Dropout service");
        try {
...
//            VaccinatorAlarmReceiver.completeWakefulIntent(intent);
    ...
    private void generateBirthRegistrationIndicators() {
        try {
            EventClientRepository eventClientRepository = ZeirApplication.getInstance().eventClientRepository();
            ...
            String lastProcessedDate = ZeirApplication.getInstance().context().allSharedPreferences().getPreference(COVERAGE_DROPOUT_BIRTH_REGISTRATION_LAST_PROCESSED_DATE);
            ...
            for (Map<String, String> result : results) {
                ...
                ZeirApplication.getInstance().context().allSharedPreferences().savePreference(COVERAGE_DROPOUT_BIRTH_REGISTRATION_LAST_PROCESSED_DATE, updatedAt);
            }
    ...
    private void generateVaccineIndicators() {
        try {
            ...
            SQLiteDatabase db = ZeirApplication.getInstance().getRepository().getWritableDatabase();
            String lastProcessedDate = ZeirApplication.getInstance().context().allSharedPreferences().getPreference(COVERAGE_DROPOUT_VACCINATION_LAST_PROCESSED_DATE);
            ...
            for (Map<String, String> result : results) {
                try {
                    ...
                    if (StringUtils.isNotBlank(dobString) && StringUtils.isNotBlank(baseEntityId) && StringUtils.isNotBlank(eventDate) && StringUtils.isNumeric(eventDate)) {
                        long timeStamp = Long.parseLong(eventDate);
                    ...
                    ZeirApplication.getInstance().context().allSharedPreferences().savePreference(COVERAGE_DROPOUT_VACCINATION_LAST_PROCESSED_DATE, updatedAt);
                } catch (Exception e) {
                    Timber.e(e);
                }
            }
    ...
    private static void updateCohortRegistrations(String baseEntityId, Date dob) {
        CohortRepository cohortRepository = ZeirApplication.getInstance().cohortRepository();
    ...
    private static void updateCumulativeRegistrations(String baseEntityId, Date vaccineDate) {
        CumulativeRepository cumulativeRepository = ZeirApplication.getInstance().cumulativeRepository();
    ...
    private static boolean isVaccineValidForCohort(String vaccine, Date dob, Date eventDate) {
        Date endDate = AppUtils.getCohortEndDate(vaccine, dob);
